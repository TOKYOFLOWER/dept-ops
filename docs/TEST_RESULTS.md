...
- **E6** 今朝のサマリーバーが表示され、3秒で全体の温度感が分かる
  → **PASS（自動・jsdom）** 同ファイル > "E6: サマリーバーに部署の状態件数と承認待ち件数が文字で表示される" / "E6: 承認待ちが0件のときは「なし」と表示する" / "サマリーバーは取得エラー時に非表示になる"
- **E7** 部署カードの3層化（根拠の折りたたみ・今日のアクションの承認状態別出し分け）
  → **PASS（自動・jsdom）** 同ファイル > "E7: 根拠（report本文＋取得時刻）はdetails要素で折りたたまれ、デフォルトで閉じている" / "E7: 承認済みの提案は「✅実行OK」として表示される" / "E7: 承認待ちの提案は「⏳社長の承認待ち」として表示される" / "E7: 却下された提案はカードに表示されない" / "E7: needs_decision=falseの提案は承認不要の情報として常に表示される"
  補足: 承認状態との突き合わせは `gas/orchestrator.js` の `attachApprovalIds_` が付与する `approval_id`
  を使う（`tests/orchestrator.test.mjs` > "要承認の提案にはapproval_idが付与され、報告履歴と承認キューの
  両方に同じIDで記録される" でPASS確認済み）。
- **E8** 専門用語（CTR・掲載順位・表示回数・薬機法リスク語・在庫API等）のタップ説明
  → **PASS（自動・jsdom）** 同ファイル > "E8: 専門用語がタップ可能な要素になっている（title属性だけに依存しない）" / "E8: 用語をタップすると説明ポップアップが開き、他の場所をタップすると閉じる"
- **E9** 承認キュータブ・状況推移ドット・アクセシビリティ
  → **PASS（自動・jsdom）** 同ファイル > "E9: 承認キューはpending/approved/rejectedタブで切り替えて一覧表示できる" / "E9: 承認待ちが0件のタブでは分かりやすい空状態メッセージが出る" / "部署カードに直近の状況推移がドット列（絵文字＋テキストラベル）で表示される" / "アクセシビリティ: スタイルシート中のfont-sizeはすべて14px相当以上である"（静的解析でCSSの全font-size宣言を検査）
  375px幅での崩れなしは上記E4と同じ手動確認手順でカバーする。
...
5. **docs全体をレビューし、公開に不適切な内部情報を汎用表現に修正**:
   発注者個人の呼称、社外の関係者・プロジェクトを具体的に指す記述を「経営者」
   「既存の社内自動化資産」等の汎用表現に置き換えた（gas/setup.js内のClaude向けプロンプト文中の
   個人の呼称も同様に修正、コード動作への影響はなし）。株式会社東京フラワーという社名自体は
   リポジトリ所有者（GitHub Organization: TOKYOFLOWER）と一致するため残している。
   金額・具体的な取引先名の記載はレビューの結果見つからなかった。
   （置き換え前の個別の語句そのものはここには記載しない）
6. 上記変更後、`npm test` で62件全PASSを再確認し、`clasp push` でGASプロジェクトへ反映済み。

## スタッフ向けUI刷新・コレクター修正に伴う変更（2026-07-07）

### タスクA: dashboard/index.html刷新

木札モチーフ（縦書き部署名の背景タグ）は維持したまま、以下を追加した。詳細な検証結果はE6〜E9を参照。

- 今朝のサマリーバー（`#summaryBar`）、部署カードの3層化（`headline`常時表示／`<details class="evidence">`
  で根拠を折りたたみ／`今日のアクション`で承認状態別に出し分け）、専門用語のタップ説明
  （`GLOSSARY`定数＋`.term`ボタン＋`#tooltipPopup`）、承認キュータブ（`#approvalArea`）、
  状況推移ドット列（`buildDotsHtml`）を実装。
- 承認状態と提案の突き合わせのため、`gas/orchestrator.js` に `attachApprovalIds_` を追加し、
  needs_decisionな提案にだけ `approval_id` を事前付番して報告履歴・承認キューの両方に
...
- E4（375px崩れなし）・E9のうち375px部分は上記の手動手順で確認する。以下はモックデータでの
  ...
  4. 「今日のアクション」欄で、承認済み提案が緑系の「✅実行OK」、承認待ちが青系の「⏳社長の承認待ち」で
     色分け表示されること、それぞれに文字ラベルが付いていることを確認
  5. report本文中の「CTR」「掲載順位」などの用語（下線付きボタン）をタップ→説明ポップアップが開き、
     「閉じる」ボタンや画面の別の場所をタップすると閉じることを確認
  6. ページ下部の「承認キュー」で「承認待ち」「承認済み」「却下」タブを切り替え、それぞれの一覧が
     切り替わることを確認（承認待ちタブには「📨 社長のLINE WORKSに依頼済み」の注記があること）
