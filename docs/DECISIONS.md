...
## 8. RMS 在庫API・CouponAPIのエンドポイント仕様（2026-07-08: 公式仕様で確定・旧仮説を置き換え）

前回セッションでは `fetchInventoryMap_`（在庫API 2.1想定、`POST /es/2.1/inventories/manage-numbers/batch/get`）
と `collectMarket_`（CouponAPI 2.0をPOST+JSONボディと仮定）を、他API呼び出しパターンからの
類推による**仮説**として実装していた。今回、公式仕様が判明したため以下の通り確定修正した
（推測は含まない。ユーザー指示に基づく確定仕様）。

- **CouponAPI**: `GET https://api.rms.rakuten.co.jp/es/1.0/coupon/search`（2.0は存在しない）。
  パラメータは `couponName/couponCode/itemUrl/couponStartDate/couponEndDate/hits/page` のみ
  （`couponStatus` は仕様に存在しないため送らない）。レスポンスはXML（`text/xml`）で
  `result > coupons > coupon` の `couponName / couponEndDate / discountType(1:定額値引 2:定率 4:送料無料) /
  discountFactor / issueCount / getCount / availCount` を読む。`couponEndDate` が現在時刻以降のものだけを
  有効クーポンとして扱う。XMLパースは `XmlService.parse()` を使用（`gas/collectors.js` の `parseCouponXml_`）。
- **在庫API**: `GET https://api.rms.rakuten.co.jp/es/2.1/inventories/manage-numbers/{manageNumber}/variants/{variantId}`
  （バリアント単位・JSONレスポンスの `quantity` が在庫数）。`items/search` の結果から
  manageNumberとvariantsのキー（SKU）を集めてこのエンドポイントを呼ぶ方式に変更
  （`gas/collectors.js` の `fetchVariantQuantity_` / `determineStockStatuses_`）。
  実行時間対策として在庫API呼び出しは実行全体で最大200バリアントに打ち切り、打ち切り時は
  報告に「200SKUまでサンプリング」と明記する。404/403（および他の例外）を返したSKUは
  「在庫不明」としてカウントから除外し、在庫ゼロと誤判定しない設計は維持している。
- テストは `tests/collectors.test.mjs`（XML解析・discountType変換・couponEndDateフィルタ・
  variant単位の在庫判定・200SKU上限・エラー時のHTTPコード/URL表示）で自動検証済み（27件）。
  `tests/sandbox.mjs` に jsdom の `DOMParser` を利用した `XmlService.parse()` スタブを追加した。

## 9. ダッシュボードの状況推移ドット列「直近14日」制約（2026-07-08: クローズ）

タスクAでは各部署カードに直近14日分の状況推移ドットを表示する仕様としたが、当時の
`gas/webapi.js` の `mapHistoryRows_` は受け入れ基準E1で定義された「履歴30件」を
**全部署合算**でそのまま踏襲しており、3部署が毎日実行される運用では1部署あたり実質10日分
程度しか遡れない可能性があった。

**対応**: `mapHistoryRows_` を「全部署合算30件」から「部署ごとに直近14件」を残す方式へ変更した。
シートは追記順（古い→新しい）のため、部署IDでグルーピングしてから各グループの末尾14件を取り、
最後にタイムスタンプ降順（newest-first）へソートして返す（`pickLatestByDept_` 等の既存の
「配列の先頭が最新」という前提はそのまま維持）。これにより各部署は自身の最新14件を必ず保持でき、
ドット列は常に最大14日分を表示できるようになった（部署の実行頻度が毎日である前提）。
acceptance_criteria.md のE1は「履歴（部署ごと直近14件）」に更新し、`tests/webapi.test.mjs` の
関連テストも更新済み。**この決定はクローズ済み**（旧「How to apply」の対応が完了）。
