...
// エラー時表示・スタッフ向けUI（サマリーバー・3層カード・用語タップ説明・承認キュータブ・
// 状況推移ドット）を検証する（375pxレイアウト崩れの目視確認は docs/TEST_RESULTS.md の手動手順を参照）。
...
// dashboard/config.js（gitignore対象で外部ファイル読み込みができない）の代わりに、
// <script src="config.js"></script> をテスト用のインライン設定に差し替える。
function withInlineConfig(config) {
  const inline = '<script>window.DEPT_OPS_CONFIG = ' + JSON.stringify(config) + ';</script>';
  const replaced = RAW_HTML.replace('<script src="config.js"></script>', inline);
  assert.notEqual(replaced, RAW_HTML, 'config.js の読み込みタグが見つからない（index.html の構造が変わった？）');
  return replaced;
}
...
// ---------- config.js 経由のURL組み立て ----------
test('config.js の gasUrl/dataKey が action=data のfetch URLに正しく反映される', async () => {
