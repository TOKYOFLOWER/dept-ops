...
2. Apps Scriptエディタで、gas/ 配下の7つのスクリプトファイル（config / setup / collectors / claude / notify / orchestrator / webapi）を1つずつ作成して貼り付ける
...
3. ダッシュボードのHTMLファイルを追加する
   - 左メニューの「ファイル」の＋ボタン →「HTML」→ ファイル名を `dashboard` と入力すると `dashboard.html` が作られる（スクリプトではなくHTMLを選ぶこと）
   - gas/dashboard.html の中身をそのまま貼り付けて保存
4. マニフェストファイル（appsscript.json）を編集する
...
5. 関数 `initSheets` を実行する
   - エディタ上部の関数選択プルダウンで `initSheets` を選ぶ → ▶実行
   - 初回は「承認が必要です」ダイアログが出るので、自分のGoogleアカウントで許可（Search Console readonly 含む・下記6）
   - 実行後、スプレッドシートに4シート（部署定義／報告履歴／承認キュー／実行ログ）と部署定義3行（market/seo/items）が生成されていることを確認
6. 初回実行時にGoogle認可（Search Console readonly を含む）を許可
...
| DATA_KEY | ダッシュボード（`action=data` / `action=dashboard`）用アクセスキー。ランダムな文字列（例: `openssl rand -hex 16` の出力） |
| WEBAPP_URL | 手順3のデプロイ後に設定（それまでは未設定でOK） |
| DASHBOARD_URL | 手順4で `WEBAPP_URL` + `?action=dashboard&key=` + `DATA_KEY` の形で設定（それまでは未設定でOK。未設定でもChatwork報告は「(未設定)」と表示されるだけで動作する） |
...
## 4. ダッシュボード（GAS配信方式・GitHub Pages不要）

ダッシュボードは静的サイトとして別途公開する必要はない。GAS自身が `doGet?action=dashboard` で
`gas/dashboard.html` を直接配信する（`gasUrl`＝`WEBAPP_URL`、`dataKey`＝`DATA_KEY` をサーバー側で
テンプレートに注入して返すため、クライアント側に設定ファイルを置く必要がない）。

1. 次のURLがダッシュボードの閲覧用URL: `（手順3のWEBAPP_URL）?action=dashboard&key=（DATA_KEY）`
2. このURLを Script Properties の `DASHBOARD_URL` に設定する（Chatwork報告の末尾リンクに使われる）
3. ブラウザで手順1のURLを開き、ダッシュボードが表示されることを確認する
   - `key` がScript Propertiesの `DATA_KEY` と一致しない場合・未指定の場合は、内容を一切含まない
     エラーページが表示される（`gasUrl`/`DATA_KEY`などは漏れない）

**注意**: このURL（`key`クエリを含む）を知っていれば誰でも報告データを閲覧できる。
社内での共有先を限定する、DATA_KEYを長いランダム値にする、等で運用すること。
...
2. `test_runMarketOnly()` を実行 → 実行完了後、報告履歴シートに market の行が追加され、実行ログにエラーがないことを確認
   - あわせて `test_runItemsOnly()` も実行しておくとよい（商品管理部だけ単独実行される）。
     在庫API（`fetchVariantQuantity_`）が失敗する場合、実行ログに最初の1件だけHTTPコード・
     試行URL・レスポンス本文（先頭200字）がWARNとして記録されるので、そこから原因を確認できる
...
6. `DASHBOARD_URL`（手順4で設定したURL）を開き、3部署のカードとサマリーバー（承認待ち件数）が表示されることを確認。スマホでも開いて崩れないことを目視確認（375px相当。docs/TEST_RESULTS.md のE4手順を参照）
...
- **ダッシュボードが「データを取得できませんでした」と出る**: Script Properties の WEBAPP_URL が正しいデプロイURLになっているか、Webアプリのアクセス権限が「全員」になっているかを確認
- **ダッシュボードのURLを開いても「アクセスできません」と出る**: URLの `key` クエリと Script Properties の DATA_KEY が完全に一致しているか確認
